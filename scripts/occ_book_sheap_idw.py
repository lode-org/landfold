#!/usr/bin/env python3
"""Compact-support kNN IDW of E-E_GM on the SHEAP dual-funnel plane.

Re-paints the IMQ SHEAP map with the same IDW kernel as
occ_book_soap_idw.py. If the GM teal well and ico blue well survive
at 0 and 0.676, the points themselves are low E; if they vanish
under k=1, IMQ invented them.
...
GM_IDX = 0
ICO_IDX = 40
# True ico-GM is 0.676. Teal / blue on ruhi_pes.
TRUE_ICO = ICO_E - GM_E
GM_WELL = 0.55
ICO_WELL = 1.50
VMAX = 5.0
...
def main() -> None:
    ...
    print(
        ...
        f"z[ico]={float(z[ico]):.6f}  true_ico={TRUE_ICO:.6f}"
    )
    ...
    for ax, k in zip(axes, KS):
        ...
        data_ico = bool(np.isfinite(fico) and abs(fico - TRUE_ICO) <= 0.05)
        survive = gm_ok and ico_ok
        print(
            f"k={k} F(GM)={fgm:.4f} F(ico)={fico:.4f} NN_E(GM)={nn_e:.4f} "
            ...
            f"at_0_and_0.676={'yes' if (data_gm and data_ico) else 'no'}"
        )
        mesh = paint(
            ax,
            xy,
    ...
    fig.suptitle(
        r"SHEAP dual-funnel   compact-support kNN IDW of $E-E_{\mathrm{GM}}$",
        fontsize=12,
    )
