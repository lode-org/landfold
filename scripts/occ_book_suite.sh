...
"$LANDFOLD" landmarks -D 10 -n 200 --seed 1 <"$CV" >"$OUT/${LABEL}.lm"
...
run phate "$LANDFOLD" embed -D 10 -d 2 -w --phate <"$CV"
run pacmap "$LANDFOLD" embed -D 10 -d 2 -w --pacmap <"$CV"
...
if [ -s "$REFS" ]; then
  run axis "$LANDFOLD" embed -D 10 -d 2 --axis "$REFS" <"$CV"
