#!/usr/bin/env python3
"""Two-panel LJ38 figure: leftover occupancy invert vs SHEAP energy IDW.

Left: leftover-well occupancy invert of the 4042 Elja book on
lj38_asinh_cer.proj (else ceriotti / asinh). F(GM)=0.32, F(ico)=0.01.
Right: SHEAP dual-funnel plane (sheap.xy), compact-support IDW of
E-E_GM. F(GM)=0, F(ico)=0.68.
...
FIG = OUT / "elja_occ_lj38_occ_vs_sheap.png"
SRC_DIRS = (
    Path("/tmp/landfold-occ-from-terra/landfold-occ-book"),
    Path("/tmp/landfold-occ-book"),
)
OCC_PROJS = (
    ("lj38_asinh_cer.proj", "occupancy invert"),
    ("lj38_ceriotti.proj", "occupancy invert"),
    ("lj38_asinh.proj", "occupancy invert"),
)
...
def main() -> None:
    ...
    for name, xy in (("occ", xy_occ), ("sheap", xy_sheap)):
    ...
    mesh_h = paint_energy(axes[1], gx_h, gy_h, e_sheap)
    mark(axes[1], xy_sheap[GM_IDX], xy_sheap[ICO_IDX])
    axes[1].set_title(f_title("SHEAP energy", fgm_h, fico_h, digits=2), fontsize=11)
    cb_h = fig.colorbar(mesh_h, ax=axes[1], fraction=0.046, pad=0.03)
